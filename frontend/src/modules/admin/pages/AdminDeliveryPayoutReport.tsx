...
import {
  getDeliveryBoys,
  getDeliveryPayoutReport,
  DeliveryBoy,
  DeliveryPayoutRow,
  DeliveryPayoutAssignment,
  DeliveryPayoutSummary,
...
const formatCommissionBasis = (a: DeliveryPayoutAssignment) => {
  if (a.commissionType === 'Percentage') {
    return `${a.commissionRate ?? 0}% of ₹${(a.commissionBasisAmount ?? 0).toFixed(2)}`;
  }
  if (a.commissionType === 'Fixed') {
    return `Fixed ₹${a.commissionRate ?? 0}`;
  }
  return '—';
};

const AdminDeliveryPayoutReport = () => {
  const { isAuthenticated, token } = useAuth();
  const [perDeliveryBoy, setPerDeliveryBoy] = useState<DeliveryPayoutRow[]>([]);
  const [summary, setSummary] = useState<DeliveryPayoutSummary | null>(null);
  const [assignments, setAssignments] = useState<DeliveryPayoutAssignment[]>([]);
  ...
  const fetchReport = async () => {
    try {
      ...
      const response = await getDeliveryPayoutReport(params);
      ...
      console.error("Error fetching delivery payout report:", error);
      toast.error("Failed to load delivery payout data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(perDeliveryBoy.map(row => ({
      ...
      "Mobile": row.mobile || "",
      "Current Commission Config": row.commissionType === 'Percentage' ? `${row.commission}%` : `Fixed ₹${row.commission ?? 0}`,
      "Deliveries": row.deliveredCount,
      "Total Payout": row.totalPayout.toFixed(2),
      "Avg / Delivery": row.avgPerDelivery.toFixed(2),
    })));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Delivery Payout");
    XLSX.writeFile(workbook, `Delivery_Payout_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  const downloadPDF = () => {
    const doc = new jsPDF('portrait');
    doc.setFontSize(18);
    doc.text('Delivery Payout Report', 14, 15);
    doc.setFontSize(10);
    ...
    const tableData = perDeliveryBoy.map(row => [
      row.name || "Unknown",
      row.mobile || "",
      row.commissionType === 'Percentage' ? `${row.commission}%` : `Fixed ₹${row.commission ?? 0}`,
      row.deliveredCount.toString(),
      `₹${row.totalPayout.toFixed(2)}`,
      `₹${row.avgPerDelivery.toFixed(2)}`,
    ]);

    autoTable(doc, {
      head: [['Delivery Boy', 'Mobile', 'Commission Config', 'Deliveries', 'Total Payout', 'Avg / Delivery']],
      body: tableData,
      startY: 28,
      styles: { fontSize: 9 },
    ...
    doc.save(`Delivery_Payout_Report_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-black text-gray-900">Delivery Payout Report</h1>
              <p className="text-xs text-gray-500 font-bold mt-0.5">Track what's owed to each delivery boy, based on their actual commission config</p>
            </div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-6">
        ...
        {summary && (
          <div className="grid grid-cols-2 md:grid-cols-2 gap-4 mb-6">
            {[
              { label: "Deliveries Completed", value: summary.deliveredCount },
              { label: "Total Payout", value: `₹${summary.totalPayout.toFixed(2)}` },
            ].map((tile) => (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="text-sm font-black text-gray-900">Per-Delivery-Boy Payout Summary</h2>
          </div>
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="px-4 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Current Config</th>
                  <th className="px-4 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Deliveries</th>
                  <th className="px-4 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Total Payout</th>
                  <th className="px-4 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Avg / Delivery</th>
                </tr>
              </thead>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-16 text-center">
                      ...
                      <p className="text-gray-400 font-black text-[10px] uppercase tracking-[0.2em]">Loading payout data...</p>
                    </td>
                  </tr>
                ) : perDeliveryBoy.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-16 text-center text-gray-400 font-black italic tracking-widest uppercase text-[10px]">No completed deliveries found</td></tr>
                ) : (
                  perDeliveryBoy.map((row) => (
                    <tr key={row.deliveryBoyId} className="hover:bg-[var(--primary-alpha-10)]/30 transition-colors">
                      ...
                      <td className="px-4 py-4 font-black text-gray-900 text-sm">{row.deliveredCount}</td>
                      <td className="px-4 py-4 font-black text-emerald-600 text-sm">₹{row.totalPayout.toFixed(2)}</td>
                      <td className="px-4 py-4 font-bold text-gray-700 text-xs">₹{row.avgPerDelivery.toFixed(2)}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        {/* Delivery drill-down table */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-sm font-black text-gray-900">Delivery Drill-Down</h2>
            ...
            <table className="w-full text-sm min-w-[1100px]">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="px-4 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Commission Basis</th>
                  <th className="px-4 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Delivered At</th>
                  <th className="px-6 py-4 text-right text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Payout</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="px-6 py-16 text-center text-gray-400 font-black italic tracking-widest uppercase text-[10px]">No deliveries found</td></tr>
                ) : (
                  assignments.map((a) => (
                    <tr key={a._id} className="hover:bg-[var(--primary-alpha-10)]/30 transition-colors">
                      ...
                      <td className="px-4 py-4 font-bold text-gray-700 text-xs">{formatCommissionBasis(a)}</td>
                      <td className="px-4 py-4 font-bold text-gray-500 text-xs">{a.deliveredAt ? new Date(a.deliveredAt).toLocaleString() : "—"}</td>
                      <td className="px-6 py-4 text-right font-black text-emerald-600 text-sm">₹{(a.commissionAmount ?? 0).toFixed(2)}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        {pagination.pages > 1 && (
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
            <div className="text-xs font-black text-gray-400 uppercase tracking-widest">
              Showing <span className="text-[var(--primary-dark)]">{assignments.length}</span> of <span className="text-[var(--primary-dark)]">{pagination.total}</span> Deliveries
            </div>
            <div className="flex items-center gap-2">
              <button
...
export default AdminDeliveryPayoutReport;
