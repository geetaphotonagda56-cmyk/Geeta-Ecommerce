...
export default function ToyStore() {
  ...
  const { products, loading } = useCuratedStoreProducts('toy', 'toys');
          ...
          <div className="px-3 py-2 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <ShareButton
                iconOnly
                title="Toy Store"
                text="Check out Toy Store on Geeta Stores"
                imageUrl={`${window.location.origin}/assets/shopbystore/toy.jpg`}
                className="w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-lg bg-white/70 shadow-sm hover:bg-white/80 transition-colors border border-white/20"
              />
            </div>
          </div>
