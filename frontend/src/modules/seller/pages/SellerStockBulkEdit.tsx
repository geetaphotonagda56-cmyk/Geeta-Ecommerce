...
import { uploadImage, uploadImageFromUrl } from "../../../services/api/uploadService";
...
export default function SellerStockBulkEdit({
  ...
  const applySearchedImages = async (urls: string[]) => {
      ...
      // Only the first uploaded image takes over the replace target; any
        ...
        uploaded = await Promise.all(
          urls.map(async (url) => {
            try {
              ...
              // seller from using an image their own browser can render fine
      ...
      if (imageSourceModalVariationIndex !== null) {
          ...
          setEditableProducts((prev) => {
              ...
              if (replaceIndex != null) {
                ...
                // position (and which slot is "Main") is preserved. Any
                // extra picked images beyond the first still get appended.
                ...
                if (replaceIndex === 0) {
                  ...
                  gallery[replaceIndex - 1] = first.url;
      ...
      setEditableProducts((prev) => {
          ...
          const replaceAt = replaceImageId
            ? currentProduct.images.findIndex((img) => img.id === replaceImageId)
            : -1;
          const [firstNew, ...restNew] = newImages;
          const images = replaceAt !== -1
            ? [
                ...currentProduct.images.map((img, i) => (i === replaceAt ? firstNew : img)),
                ...restNew,
              ]
            : [...currentProduct.images, ...newImages];
  ...
  const handleAttachExistingProduct = async (productIndex: number, searchResult: any) => {
    ...
    // sellers, who can manually reactivate it - removing the variation later
  ...
  const handleVariationImageChange = async (productIndex: number, variationIndex: number, files: FileList | null, replaceIndex?: number) => {
      if (!files || files.length === 0) return;
      let isFirst = true;
      for (const file of Array.from(files)) {
        const targetReplaceIndex = isFirst ? replaceIndex : undefined;
        isFirst = false;
        try {
          ...
          setEditableProducts((prev) => {
            ...
            if (!oldProd) return prev;
            const variations = [...(oldProd.variations || [])];
            const targetVariation = { ...(variations[variationIndex] || {}) };
            if (targetReplaceIndex != null) {
              ...
              if (targetReplaceIndex === 0) {
                ...
                const gallery = [...(targetVariation.galleryImages || [])];
                gallery[targetReplaceIndex - 1] = url;
                targetVariation.galleryImages = gallery;
              }
            ...
            variations[variationIndex] = targetVariation;
            const newProd = { ...oldProd, variations, isChanged: true };
            updated[productIndex] = newProd;
            upsertEditedCache(newProd);
            return updated;
  ...
  const filteredProducts = useMemo(() => {
    ...
    return editableProducts.filter(p => {
      ...
      const variationsMatch = term !== "" && Array.isArray(p.variations) && p.variations.some((v: any) => {
        if (norm(String(v?.name || "")).includes(term)) return true;
        if (norm(String(v?.value || "")).includes(term)) return true;
        if (norm(String(v?.sku || "")).includes(term)) return true;
        const vBarcodes = Array.isArray(v?.barcode) ? v.barcode : (v?.barcode ? [v.barcode] : []);
        return vBarcodes.some((b: string) => norm(String(b)).includes(term));
      });
      const nameMatch = norm(p.productName).includes(term) || barcodeMatch || variationsMatch;
  ...
  const renderVariationBodyCell = (
    ...
    // Rack is handled separately below via a lock/custom toggle, since it
    // has its own "same as parent" default regardless of variation count.
    ...
    switch (key) {
      ...
      case "rackNumber": {
        ...
        const rackValue = isCustom ? (variation.rackNumber || "") : (product.rackNumber || "");
        return (
          <td key={key} className="p-0 border-r border-neutral-200">
            <div className="flex items-center h-full">
              <button
                ...
                title={isCustom ? "Custom rack - click to lock to parent product" : "Locked to parent product's rack - click to customize"}
              >
                {isCustom ? (
              ...
              <input
                type="text"
                disabled={!isCustom}
                className="flex-1 h-full px-2 py-2 bg-transparent border-none text-sm disabled:text-neutral-400 disabled:cursor-not-allowed"
                value={rackValue}
                onChange={(e) => handleVariationFieldChange(originalIndex, variationIndex, "rackNumber", e.target.value)}
              />
            </div>
          </td>
        );
