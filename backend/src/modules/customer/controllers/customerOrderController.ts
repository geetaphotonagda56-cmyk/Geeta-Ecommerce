...
export const createOrder = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Notify sellers/admin of the new order. Delivery boys are NOT
        // broadcast to anymore - admin now manually confirms and dispatches
        // orders via the Order Delivery workflow. notifyDeliveryBoysOfNewOrder
        // is still used as a fallback by deliveryEscalationScheduler if admin
        // fails to dispatch in time.
        try {
            ...
            console.error("Error notifying sellers/admin:", notificationError);
        }
...
export const verifyOnlinePayment = async (req: Request, res: Response) => {
    try {
        ...
        // Notify sellers/admin. Delivery boys are not broadcast to - admin
        // manually confirms and dispatches via the Order Delivery workflow.
        const io: SocketIOServer = (req.app.get("io") as SocketIOServer);
        if (io) {
            await notifySellersOfOrderUpdate(io, order, 'NEW_ORDER');
            await notifyAdminsOfNewOrder(io, order);
        }
